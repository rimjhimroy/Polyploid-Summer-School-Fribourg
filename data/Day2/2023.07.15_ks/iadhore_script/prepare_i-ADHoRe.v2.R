...
prepare_gene_list <- function(species, gff_file, outdir){
    ...
    for( i in names(split_gff) ){
        ...
        filepath <- file.path("gene_lists", species, filename)
...
for( i in seq_len(length(iadhore_sp_list)) ){
    ...
    config_dir <- paste0("i-adhore.", species, "_vs_", species)
    make_configure_iadhore(species, species_gene_list, iadhore_conf_file, config_dir)
    writeLines(paste0("i-adhore ", "i-adhore.", species, "_vs_", species, "/", species, "_vs_", species, ".i-adhore.ini"), config_file_con)
    for( j in seq_len(length(iadhore_sp_list)) ){
        if( j > i ){
            ...
            config_dir <- paste0("i-adhore.", species1, "_vs_", species2)
            ...
            writeLines(paste0("i-adhore ", "i-adhore.", species1, "_vs_", species2, "/", species1, "_vs_", species2, ".i-adhore.ini"), config_file_con)
        }
    }
}
close(config_file_con)
